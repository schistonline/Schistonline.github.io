...
const SearchPage = {
    ...
    searchHistory: [],
    popularSearches: [],
    searchType: 'products', // 'products' or 'suppliers'
    ...
    setSearchType(type) {
        ...
        if (form) {
            if (type === 'products') {
                form.action = 'B2B-search-results.html';
            } else {
                form.action = 'supplier-results.html';
            }
        }
    ...
    setupEventListeners() {
        ...
        if (searchForm) {
            searchForm.addEventListener('submit', (e) => {
                ...
                // Form will naturally submit to the appropriate action
            });
        }
            ...
            var historyHtml = historyData.data.map(item => {
                ...
                return `
                    <div class="history-item">
                        <a href="${this.searchType === 'products' ? 'B2B-search-results.html' : 'supplier-results.html'}?q=${encodeURIComponent(item.query)}" class="history-link">
            ...
            var searchesHtml = popularSearches.map(search => {
                return `
                    <div class="popular-item">
                        <a href="${this.searchType === 'products' ? 'B2B-search-results.html' : 'supplier-results.html'}?q=${encodeURIComponent(search)}" class="popular-link">
    ...
    async loadQuickCategories() {
        try {
            ...
            var categoriesHtml = categoriesData.data.map(cat => {
                ...
                // For categories, we default to product search
                return `
